...
public class LogUtils {
    private static DateFormat currentDateFormatter = new SimpleDateFormat("yyyy-MM-dd");
    private LogUtils() {
    ...
    private static String type            = "common";
     ...
     * @param tag            标签
     * @param type           例如图片、错误日志、文件等等
     */
    public static void init(Context context, boolean logSwitch, boolean log2FileSwitch, char logFilter, String tag, String type) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            dir = Environment.getExternalStorageDirectory()+ File.separator + "zwjk"+ File.separator+LogUtils.type+ File.separator+currentDateFormatter.format(new Date())+ File.separator;
        } else {
            dir = context.getCacheDir().getPath() + File.separator + "zwjk"+ File.separator+LogUtils.type+ File.separator+currentDateFormatter.format(new Date())+ File.separator;
        }
    ...
    /**
     * 获取LogUtils建造者
     * <p>与{@link #init(Context, boolean, boolean, char, String, String)}两者选其一</p>
     *
     * @param context 上下文
     * @return Builder对象
     */
